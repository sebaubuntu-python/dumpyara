...
class GPTSlice(object):
    ...
    def display(self, idx):
        ...
        verbose(
            'Name({:d}): "{:s}" start={:d} end={:d} count={:d}'.format(
                idx, self.name, self.startLBA, self.endLBA, self.endLBA - self.startLBA + 1
            )
        )
    ...
    def __init__(self, buf):
        ...
        data = dict(zip(self._gpt_slice_fmt.keys(), self._gpt_struct.unpack(buf)))
...
class GPT(object):
    ...
    def display(self):
        ...
        verbose(
            "myLBA={:d} altLBA={:d} dataStart={:d} dataEnd={:d}".format(
                self.myLBA, self.altLBA, self.dataStartLBA, self.dataEndLBA
            )
        )
        ...
        if self.myLBA == 1:
            if self.entryStart != 2:
                verbose(
                    "Note: {:d} unused blocks between GPT header and entry table".format(
                        self.entryStart - 2
                    )
                )
            ...
            if endEntry < self.dataStartLBA:
                verbose(
                    "Note: {:d} unused slice entry blocks before first usable block".format(
            ...
            if self.entryStart != self.dataEndLBA + 1:
                verbose(
                    "Note: {:d} unused slice entry blocks after last usable block".format(
    ...
    def tryParseHeader(self, buf):
        ...
        data = dict(
            zip(self._gpt_head_fmt.keys(), self._gpt_struct.unpack(buf[0 : self._gpt_size]))
        )
