    ...
    def loadHeader(self, file):
        ...
        for key in self._dz_collapsibles:
            if type(dz_item[key]) is str or type(dz_item[key]) is bytes:
                ...
                if dz_item[key] != 0:
                    LOGD(
                        '[!] Error: Value supposed to be zero in field "'
                        + key
                        + '" is non-zero ('
                        + hex(dz_item[key])
                        + ")"
...
class UNDZChunk(dz.DZChunk, UNDZUtils):
    ...
    def display(self, sliceIdx, selfIdx):
        ...
        if cmd.batchMode:
            ...
            LOGD(
                "{:2d}/{:2d} : {:s} ({:d} bytes)".format(
                    sliceIdx, selfIdx, self.chunkName.decode("utf8"), self.dataSize
                )
            )
        self.Messages()
        return ++selfIdx
    ...
    def extract(self):
        ...
        if md5.digest() != self.md5:
            LOGD(
                "[!] Error: MD5 of data doesn't match header ({:32s} vs {:32s})".format(
                    md5.hexdigest(), b2a_hex(self.md5)
                )
            )
            sys.exit(1)
    ...
    def __init__(self, dz, file):
        ...
        if dz_item["targetSize"] & 0x1FF != 0:
            self.messages.append(
                "[?] Warning: uncompressed size is {:d}, not a multiple of 512 (please report!)".format(
                    dz_item["targetSize"]
                )
            )
...
class UNDZSlice(object):
    ...
    def addChunk(self, chunk):
        ...
        if offset < self.start:
            LOGD(
                '[!] Warning: Chunk is part of "{:s}", but starts in front of slice?!'.format(
                    self.name
                )
            )
...
class UNDZFile(dz.DZFile, UNDZUtils):
        ...
        try:
            ...
            for slice in g.slices:
                new = UNDZSlice(
                    ...
                    index,
                    slice.name,
                    slice.startLBA << self.shiftLBA,
                    (slice.endLBA + 1) << self.shiftLBA,
                )
            ...
            for i in range(len(g.slices)):
                if next != g.slices[i].startLBA:
                    LOGD(
                        f"[!] Unallocated space found. Slice, Start, Size, End: {str(next)}, {str((g.slices[i].startLBA - next) << self.shiftLBA)}, {next << self.shiftLBA}, {(g.slices[i].startLBA - 1) << self.shiftLBA}"
                    )
                    emptycount += 1
            ...
            slice = UNDZSlice(
                self,
                index,
                self.chunks[-1].getSliceName(),
                g.dataEndLBA << self.shiftLBA,
                (g.altLBA + 1) << self.shiftLBA,
            )
            self.slices.append(slice)
    ...
    def checkValues(self):
        ...
        if md5Headers != self.md5:
            LOGD(
                "[!] Error: MD5 of chunk headers doesn't match header ({:32s} vs {:32s})".format(
                    self.md5Headers.hexdigest(), b2a_hex(self.md5)
                )
            )
            sys.exit(-1)
        ...
        if md5HeaderNZ == self.unknown2:
            ...
            self.messages.add(
                "[ ] MD5 of non-zero header areas not found ({:32s})".format(
                    self.md5HeaderNZ.hexdigest()
                )
            )
...
class DZFileTools:
    ...
    def parseArgs(self):
        ...
        parser.add_argument(
            "-f", "--file", help="DZ File to read", action="store", required=True, dest="dzfile"
        )
        parser.add_argument(
            "-b", "--batch", help="batch mode", action="store_true", dest="batchMode"
        )
        group = parser.add_mutually_exclusive_group(required=True)
        group.add_argument(
            "-l", "--list", help="list slices/partitions", action="store_true", dest="listOnly"
        )
        group.add_argument(
            "-x",
            "--extract",
            help="extract chunk-file(s) for reconstruction (all by default)",
            action="store_true",
            dest="extractChunkfile",
        )
        group.add_argument(
            "-c",
            "--chunk",
            help="extract data chunk(s) (all by default)",
            action="store_true",
            dest="extractChunk",
        )
        group.add_argument(
            "-s",
            "--single",
            help="extract diskslice(s) (partition(s)) (all by default)",
            action="store_true",
            dest="extractSlice",
        )
        group.add_argument(
            "-i",
            "--image",
            help="extract all slices/partitions as a disk image",
            action="store_true",
            dest="extractImage",
        )
        parser.add_argument(
            "-d", "--dir", "-o", "--out", help="output location", action="store", dest="outdir"
        )
    ...
    def cmdExtractChunk(self, files):
        ...
        for idx in files:
            ...
            if idx < 0 or idx >= self.dz_file.getChunkCount():
                LOGD(
                    "[!] Cannot extract out of range chunk {:d} (min=0 max={:d})".format(
                        idx, self.dz_file.getChunkCount() - 1
                    )
                )
                sys.exit(1)
    ...
    def cmdExtractChunkfile(self, files):
        ...
        for idx in files:
            ...
            if idx < 0 or idx >= self.dz_file.getChunkCount():
                LOGD(
                    "[!] Cannot extract out of range chunkfile {:d} (min=0 max={:d})".format(
                        idx, self.dz_file.getChunkCount() - 1
                    )
                )
                sys.exit(1)
    ...
    def cmdExtractSlice(self, files):
        ...
        for idx in files:
            ...
            if idx < 0 or idx > self.dz_file.getSlice(-1).getIndex():
                LOGD(
                    "[!] Cannot extract out of range slice {:d} (min=0 max={:d})".format(
                        idx, self.dz_file.getSlice(-1).getIndex()
                    )
                )
                sys.exit(1)
